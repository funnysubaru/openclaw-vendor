...
// parentPort（MessagePort 语义，用于父子进程双向通信）。vendor 不依赖 electron 类型包
// （也不该依赖——这是纯 Node 引擎，Electron 集成是 Yuiclaw 壳层的事），所以这里按 Electron
// 官方 MessagePortMain 文档手写一个只包含本文件用到的成员的最小接口，避免引入整个
// electron 类型依赖。普通 Node 进程 / Server Mode 下 process.parentPort 恒为 undefined，
// 下面的用法全部走可选链，对上游、对非 utilityProcess 运行方式零影响。
...
// 子进程 stdin 恒为 "ignore"（Electron 官方限制），上面的 stdin 暗号通道在 utilityProcess
// 模式下物理不存在。utilityProcess 父子官方通信通道是 MessagePort：父进程
// child.postMessage(msg) → 子进程 process.parentPort.on("message", ...)。
// 契约红线：以下三个常量必须与 Yuiclaw 侧 packages/gateway/src/process-tree.ts 的同名常量
// 字面量完全一致（Yuiclaw 有单测 grep 本文件断言一致）。改一侧必须同改另一侧，否则父子
// 进程握手/关闭消息互相认不出。
...
export async function runGatewayLoop(params: {
  ...
  if (process.parentPort && process.env.OPENCLAW_PARENTPORT_CONTROL === "1") {
    parentPortListener = (event) => {
      ...
      if (payload?.type === PARENTPORT_SHUTDOWN_TYPE) {
        ...
        // 必须复用现成 request("stop") 完整优雅路径——它负责释放 gateway lock、drain
        // 在途任务、按统一退出码走，且自动获得 runAcceptedRequest 里的硬退出看门狗
        // （armShutdownHardExitWatchdog）保护。这里绝不能 process.exit() 或直接杀进程
        // 绕过这条路径，否则会导致 lock 文件残留、下次启动误判"gateway 仍在运行"而拒绝启动。
    ...
    //
    // 握手本身只是「能力上报」而非关闭流程的必要前提——父进程收不到 ready 消息时会自行
    // 回退到旧的强杀路径（退化但不致命）。所以这里必须把 postMessage 包一层 try/catch：
    // 此时 gateway lock 已拿到、parentPortListener 也已装好，若 postMessage 在极端情况下
  ...
  // 说明：double-stop 幂等不需要在这里额外加互斥锁。parentPort 通道与 stdin / 信号通道
  // 最终都汇聚到同一个 request() 入口，其顶部的 shuttingDown 闸门已经保证了无论从哪条
  // 通道、触发几次关闭请求，优雅关闭流程只会真正执行一次。
